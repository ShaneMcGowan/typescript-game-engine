...
export class SCENE_GAME_MAP_TEST_PATHING_2 extends SceneMap {
  // config
  ...
  constructor(protected scene: SCENE_GAME) {
    ...
    const config = {
      ...
      movementType: MovementType.Goal,
    };

    this.scene.addObject(new NpcObject(this.scene, { x: 30, y: 1, ...config, }));
    this.scene.addObject(new NpcObject(this.scene, { x: 13, y: 3, ...config, }));
    this.scene.addObject(new NpcObject(this.scene, { x: 7, y: 7, ...config, }));
    this.scene.addObject(new NpcObject(this.scene, { x: 23, y: 5, ...config, }));
    this.scene.addObject(new NpcObject(this.scene, { x: 30, y: 15, ...config, }));
    this.scene.addObject(new NpcObject(this.scene, { x: 29, y: 16, ...config, }));
