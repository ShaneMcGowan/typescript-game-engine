  ...
  awake(): void {
    for (let i = 0; i < ITEMS_FOR_SALE.length; i++) {
      ...
      this.items.push(new ShopItemBuyObject(this.scene, {
        positionX: this.transform.position.local.x + 2 + (i * 2.5),
        positionY: this.transform.position.local.y + 2,
        price: item.price,
        type: item.type,
      }));
    }
    ...
    for (let col = 0; col < columnsPerRow; col++) {
      for (let row = 0; row < totalRows; row++) {
        ...
        this.items.push(new ShopItemSellObject(this.scene, {
          positionX: this.transform.position.local.x + 19 + (col * 2.5),
          positionY: this.transform.position.local.y + 3 + (row * 2.5),
          item: item,
          index: index,
        }));
      }
